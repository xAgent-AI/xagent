...
export class BrowserOperator extends Operator {
  ...
  protected async execute(params: ExecuteParams): Promise<ExecuteOutput> {
    ...
    this.logger.info('[BrowserOperator] execute', { action_type, startX, startY });
  ...
  private async executeAction(
    ...
    switch (actionType) {
      case 'wait':
        this.logger.info('[BrowserOperator] wait', inputs);
        await sleep(5000);
        break;

      case 'mouse_move':
      case 'hover':
        this.logger.info('[BrowserOperator] mouse_move');
      ...
      case 'left_click':
      case 'left_single':
        this.logger.info('[BrowserOperator] left_click');
        await page.mouse.move(startX, startY);
        await sleep(100);
      ...
      case 'left_double':
      case 'double_click':
        this.logger.info(`[BrowserOperator] ${actionType}(${startX}, ${startY})`);
        await page.mouse.move(startX, startY);
        await sleep(100);
      ...
      case 'right_click':
      case 'right_single':
        this.logger.info('[BrowserOperator] right_click');
        await page.mouse.move(startX, startY);
        await sleep(100);
      ...
      case 'middle_click':
        this.logger.info('[BrowserOperator] middle_click');
        await page.mouse.move(startX, startY);
      ...
      case 'type': {
        ...
        this.logger.info('[BrowserOperator] type', content);
        if (content) {
      ...
      case 'scroll': {
        ...
        switch (direction?.toLowerCase()) {
          ...
          case 'down':
            ...
            break;
          default:
            this.logger.warn(`[BrowserOperator] Unsupported scroll direction: ${direction}`);
        }
        break;
      }
      ...
      case 'navigate': {
        ...
        this.logger.info(`[BrowserOperator] navigate to ${url}`);
        await page.goto(url, { waitUntil: 'networkidle2' });
        break;
      }

      case 'navigate_back':
        this.logger.info('[BrowserOperator] navigate_back');
        await page.goBack();
        break;
      ...
      case 'finished':
      case 'user_stop':
        this.logger.info(`[BrowserOperator] ${actionType}`);
        break;

      default:
        this.logger.warn(`[BrowserOperator] Unsupported action: ${actionType}`);
    }
  }
