 ...
 * systemPrompt is generated by GUI Agent and passed to the caller
 */
export type RemoteVlmCaller = (image: string, prompt: string, systemPrompt: string) => Promise<string>;
...
`;
   ...
   * Remote mode: use remoteVlmCaller for VLM calls
   */
  private async callModelAPI(
    ...
    // === REMOTE 模式 ===
      ...
      const { image, prompt } = this.extractImageAndPrompt(messages);

      // Use externally injected VLM caller function
      const prediction = await remoteVlmCaller(image, prompt, this.systemPrompt);
